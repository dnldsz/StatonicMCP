...
// ── Types (mirrors iterate-editor/src/renderer/src/types.ts) ──────────────
...
interface VideoSegment {
  id: string; type: 'video'; src: string; name: string
  startUs: number; durationUs: number
  sourceStartUs: number; sourceDurationUs: number; fileDurationUs: number
  sourceWidth: number; sourceHeight: number
  clipX: number; clipY: number; clipScale: number
...
interface TextSegment {
  ...
  textAlign: 'left' | 'center' | 'right'; textScale: number
}

interface AudioSegment {
  id: string; type: 'audio'; src: string; name: string
  startUs: number; durationUs: number
  sourceStartUs: number; sourceDurationUs: number; fileDurationUs: number
  volume: number
  dropTimeUs?: number
...
type Segment = VideoSegment | TextSegment | AudioSegment

interface Track { id: string; type: 'video' | 'text' | 'audio'; label: string; segments: Segment[]; muted?: boolean }
interface Project { name: string; accountId?: string; canvas: { width: number; height: number }; tracks: Track[] }
...
function saveProject(path: string, project: Project): void {
  writeFileSync(path, JSON.stringify(project, null, 2))
}

function findSegment(project: Project, id: string): { seg: Segment; track: Track } | null {
...
function extractFrame(videoPath: string, timeSec: number): string {
...
function getVideoInfo(videoPath: string): { width: number; height: number; durationSec: number; rotation: number } {
...
function summariseProject(project: Project): string {
  const lines: string[] = [
...
function extractKeyframes(videoPath: string, count: number): { timestamps: number[]; frames: string[] } {
...
const server = new Server(
  { name: 'iterate-mcp', version: '0.1.0' },
  { capabilities: { tools: {} } }
)
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'read_project': {
        const project = readProject(args['path'] as string)
        return {
          content: [
            {
              type: 'text',
      ...
      case 'update_segment': {
        const { path, id, patch } = args as { path: string; id: string; patch: Record<string, unknown> }
        const project = readProject(path)
        const found = findSegment(project, id)
        if (!found) throw new Error(`Segment "${id}" not found`)
        const { seg, track } = found
        const idx = track.segments.indexOf(seg)
        track.segments[idx] = { ...seg, ...patch } as Segment
      ...
      case 'delete_segment': {
        const { path, id } = args as { path: string; id: string }
        const project = readProject(path)
        let deleted = false
        for (const track of project.tracks) {
          const idx = track.segments.findIndex(s => s.id === id)
          if (idx !== -1) { track.segments.splice(idx, 1); deleted = true; break }
        }
        if (!deleted) throw new Error(`Segment "${id}" not found`)
        saveProject(path, project)
      ...
      case 'add_text_segment': {
        const {
          path, text, start_sec, duration_sec,
          x = 0, y = 0, font_size = 80, color = '#ffffff',
          bold = false, italic = false,
          stroke_enabled = false, stroke_color = '#000000',
          text_align = 'center',
        } = args as {
          path: string; text: string; start_sec: number; duration_sec: number
          x?: number; y?: number; font_size?: number; color?: string
          bold?: boolean; italic?: boolean
          stroke_enabled?: boolean; stroke_color?: string
      ...
      case 'add_zoom_keyframes': {
        const { path, segment_id, keyframes } = args as {
          path: string
          segment_id: string
          keyframes: Array<{ time_sec: number; scale: number }>
        }
        const project = readProject(path)
        const found = findSegment(project, segment_id)
        if (!found) throw new Error(`Segment "${segment_id}" not found`)
        const { seg, track } = found
        if (seg.type !== 'video') throw new Error(`Segment "${segment_id}" is not a video segment`)

        const videoSeg = seg as VideoSegment
        const scaleKeyframes: ScaleKeyframe[] = keyframes.map(kf => ({
        ...
        const idx = track.segments.indexOf(seg)
        track.segments[idx] = { ...videoSeg, scaleKeyframes } as Segment
        saveProject(path, project)
      ...
      case 'get_frames': {
        const { video_path, times_sec } = args as { video_path: string; times_sec: number[] }
        ...
        for (const t of frames) {
          try {
            ...
            content.push({ type: 'text', text: `Failed at ${t}s: ${(e as Error).message}` })
          }
        }
        return { content }
      }
      ...
      case 'get_video_info': {
        const info = getVideoInfo(args['video_path'] as string)
        return {
          content: [{
            type: 'text',
      ...
      case 'render_preview': {
        const { project_path } = args as { project_path: string; time_sec?: number }
        const project = readProject(project_path)
        const { canvas, tracks } = project

        // Collect active segments at time_sec, sorted by track index (lower = bottom layer)
        ...
        let time_sec = (args as { time_sec?: number }).time_sec
        ...
        for (let ti = 0; ti < tracks.length; ti++) {
          for (const seg of tracks[ti].segments) {
            ...
            if (time_sec < start || time_sec >= end) continue
      ...
      case 'analyze_video_clip': {
        const { video_path, category = 'general', keyframe_count = 4, metadata: providedMetadata } = args as {
          video_path: string
          category?: string
        ...
        if (providedMetadata) {
          const fullMetadata: ClipMetadata = {
            id: uid(),
            path: video_path,
      ...
      case 'index_clip_bank': {
        const { folder_path, regenerate = false } = args as { folder_path: string; regenerate?: boolean }
      ...
      case 'search_clip_bank': {
        const { index_path, query, category } = args as {
          index_path: string
          query: string
          category?: string
        }
        ...
        const index: ClipIndex = JSON.parse(readFileSync(indexFile, 'utf-8'))
        ...
        const clipSummaries = candidates.map((c, i) =>
          `[${i}] **${c.name}** (${c.category})\n` +
          `    Path: ${c.path}\n` +
      ...
      case 'search_statonic_library': {
        const { query, category, account_id } = args as { query: string; category?: string; account_id?: string }
        ...
        if (existsSync(accountsPath)) {
          ...
          for (const accId of accounts) {
            ...
            if (account_id && accId !== account_id) continue
        ...
        const clipSummaries = clips.map((c, i) =>
          `[${i}] **${c.name}** (${c.category}${c.accountId ? ` | ${c.accountId}` : ''})\n` +
          `    Description: ${c.description}\n` +
          `    Tags: ${c.tags.join(', ')}\n` +
          `    Mood: ${c.mood} | Subject: ${c.subject_visible ? c.subject_position : 'none'} | Setting: ${c.setting}\n` +
          `    Duration: ${c.duration.toFixed(1)}s | Size: ${c.width}×${c.height}\n` +
      ...
      case 'get_clips_by_category': {
        const { category } = args as { category: 'hook' | 'gizmo' | 'showcase' }
      ...
      case 'write_statonic_project': {
        const { project, filename } = args as { project: any; filename: string }
        ...
        return {
          content: [{
            ...
            text: `✅ Project saved to: ${projectPath}\n\n` +
              ...
              `- Total segments: ${project.tracks.reduce((sum: number, t: any) => sum + t.segments.length, 0)}`,
          }],
        }
      }

      case 'analyze_statonic_library': {
        const { clip_id, metadata, force = false } = args as { clip_id?: string; metadata?: any; force?: boolean }
        ...
        if (clip_id) {
          ...
          // List all clips
          const summary = unanalyzed.map(c =>
            `• ${c.name} (${c.id}) [${c.accountId}] - ${c.duration.toFixed(1)}s, ${c.width}×${c.height}${c.analyzed ? ' ✓' : ''}`
          ).join('\n')
      ...
      case 'get_suitable_audio': {
        const { hook_duration_sec, total_duration_sec, prefer_closest = false } = args as {
      ...
      case 'get_reference_frames': {
        ...
        const { frames, totalDuration } = request as { frames: Array<{ timeSec: number; framePath: string }>; totalDuration: number }
      ...
      case 'write_reference_result': {
        let { slots, spanning_texts: rawSpanning, hookTextY, spanningTextY, slotTextY } = args as {
          slots: any[]; spanning_texts?: any[]
          hookTextY?: number; spanningTextY?: number; slotTextY?: number
        }
        let spanning_texts: any[] = rawSpanning ?? []
      ...
      case 'use_template': {
        const { template_id, slots: slotOverrides = [], project_name } = args as {
          template_id: string
          slots?: Array<{ slot_id: string; clip_id?: string; text?: string }>
          project_name?: string
        }
        const homeDir = process.env.HOME || process.env.USERPROFILE || '~'
        const templatesDir = join(homeDir, 'Library', 'Application Support', 'Statonic', 'templates')
        ...
        let currentAccountId: string | null = null
        if (existsSync(stateFile)) {
          try { currentAccountId = JSON.parse(readFileSync(stateFile, 'utf-8')).currentAccountId } catch {}
        }
        if (!currentAccountId) throw new Error('No account active in Statonic. Select an account first.')

        const clipLibraryDir = join(homeDir, 'Library', 'Application Support', 'Statonic', 'clip-library', 'accounts', currentAccountId, 'clips')
        const clipsByCategory: Record<string, Array<{ id: string; path: string; name: string; durationUs: number; width: number; height: number }>> = {}
        if (existsSync(clipLibraryDir)) {
          ...
          for (const clipId of clipIds) {
            ...
            try {
              ...
              if (!clipsByCategory[category]) clipsByCategory[category] = []
        ...
        function uid(): string { return randomBytes(4).toString('hex') }
        function pickClip(category: string): typeof clipsByCategory[string][0] | null {
        ...
        const videoTrack: any = { id: uid(), type: 'video', label: 'VIDEO', segments: [] }
        const textTrack: any = { id: uid(), type: 'text', label: 'TEXT', segments: [] }

        for (const slot of template.slots) {
          const override = slotOverrides.find((o: any) => o.slot_id === slot.slot_id)
        ...
        const filledSlots = template.slots.map((s: any) => {
          const ov = slotOverrides.find((o: any) => o.slot_id === s.slot_id)
      ...
      case 'generate_hook_options': {
        const { topic, count = 5 } = args as { topic: string; count?: number }
        const homeDir = process.env.HOME || process.env.USERPROFILE || '~'
        ...
        const formulaList = knowledge.formulas.map(f =>
          `- ${f.id}: "${f.pattern}" (e.g. "${f.example}")${f.notes ? ' — ' + f.notes : ''}`
        ).join('\n')
        const learnedList = knowledge.learned_examples.slice(0, 5).map((e: any) =>
          `- "${e.extracted_text}" (formula: ${e.formula}, topic: ${e.topic})`
        ).join('\n')
...
- Only output the JSON array, no other text`
        ...
        const formatted = options.map((o: any, i: number) =>
          `${i + 1}. [${o.formula}]\n   Text: "${o.text}"\n   Why: ${o.explanation}`
        ).join('\n\n')
      ...
      case 'learn_from_hook_video': {
        const { video_path } = args as { video_path: string }
        ...
        for (const t of timestamps) {
          try { frames.push(extractFrame(video_path, t)) } catch {}
        }
